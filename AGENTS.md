...
- Do not rely on `npm run dev` unless `nodemon` has been installed; package scripts reference it but it is not listed as a dependency.
...
- Gateway routes are assignment-facing paths like `/register/userregister`, `/auth/login`, `/admin/viewalluser`, and `/user/viewprofile`.
- Internal services use prefixed paths like `/api/registration/userregister` and `/api/login/login`; the gateway maps public paths to these internal paths.
...
- There is no lint, CI, or npm test suite in this repo.
